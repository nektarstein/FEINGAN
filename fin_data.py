...
class data_generator():
        ...
        def snp500_index(self):
                ...
                s.prepare_pd()
                table = s.get_pd_table()
                return table['Log Return'].to_numpy()  # Changed to to_numpy()
        ...
        def individuals_data_random_picker(self):
                ...
                for i in range(self.batch_size):
                        ...
                        sequence = table['Log Return'].to_numpy()  # Changed to to_numpy()
                        size = sequence.size  # Size can be directly obtained from the array
                        
                        random_pos = np.random.randint(0, size - self.sequence_length)
                        data.append([sequence[random_pos:random_pos + self.sequence_length]])
                
                data = np.array(data)
                # data /= max(data.max(), -data.min())  # Uncomment if normalization is needed
        ...
        def index_data_random_picker(self):
                ...
                size = sequence.size  # size remains the same as sequence is now a numpy array
